	...
	\While{$n_{gen} < n_{gen}^{MAX}$}{
		...
		\For{$i=1$ to $\floor{ 1 / \alpha}$}{
			\For{\emph{portion} $=1$ to $\alpha \, ($\emph{population size}$)$} {
				...
				candidates = list() \\
				\Repeat{\emph{candidate[0] respects constraints}}{
					candidates[0] $\gets$ current good point + perturbation() \\
					candidates[1] $\gets$ candidates[0] + perturbation() \\
					candidates[2] $\gets$ candidates[1] + perturbation() \\
					sort(candidates)\\
				}
				population[i + $\alpha ($population size$)$ portion] $\gets$ candidates[0] \\
			}
		}
		Increment $n_{gen}$ \\
	}
...
% TODO spiegare meglio (a livello di esposizione, il concetto è ok)
...
% mention plots -> discuss using them, not personal impressions
% adfc SA and JB were not effective
% final result plot has EDFC + ADFC, GA
\smallbreak
...
%TODO Davide scrivi quello che hai fatto per l'update della objective function
